...
python main_v7.py --seed 1
# あるいは明示
python main_v7.py --gpu 0 --seed 1
```

CPU 実行に切り替えたい場合:

```bash
python main_v7.py --device cpu --seed 1
```

出力例（自動命名）:

```
./results_v7_iter1000_batch256_seed1_cuda0/
  ├─ run_v4.log
  ├─ learning_result/
...
```bash
python main_v7.py --gpu 0 --seed 1
python main_v7.py --gpu 1 --seed 2
```

- バックグラウンドで開始（nohup + ログ標準出力 redirection）

```bash
nohup python main_v7.py --gpu 0 --seed 1 > seed1.out 2>&1 &
nohup python main_v7.py --gpu 1 --seed 2 > seed2.out 2>&1 &
```

- notify-run を使う（通知が欲しい場合）

```bash
notify-run wsl-ubuntu -- nohup python main_v7.py --gpu 0 --seed 1 > seed1.out 2>&1 &
notify-run gpu02 -- nohup python main_v7.py --gpu 1 --seed 20 > seed20.out 2>&1 &
```

出力先を明示したい場合（同名衝突を避けたいとき等）:

```bash
nohup python main_v7.py --gpu 0 --seed 1 --result-dir ./results_gpu0_seed1 > s1_g0.out 2>&1 &
nohup python main_v7.py --gpu 1 --seed 2 --result-dir ./results_gpu1_seed2 > s2_g1.out 2>&1 &
```
...
```bash
ps aux | grep main_v7.py | grep -v grep
```
...
```bash
pkill -f "main_v7.py.*--gpu 0"
# または明示 device 指定で動かしているなら
pkill -f "main_v7.py.*--device cuda:0"
```
...
- ルート: `RESULT_DIR`（例: `results_v7_iter1000_batch256_seed1_cuda0`）
...
```bash
python main_v7.py --gpu 0 --seed 1
python main_v7.py --gpu 0 --seed 2
```

- 2GPU 環境で seed を変えて同時に 2 本:

```bash
nohup python main_v7.py --gpu 0 --seed 1 > s1_g0.out 2>&1 &
nohup python main_v7.py --gpu 1 --seed 2 > s2_g1.out 2>&1 &
```
...
```bash
python main_v7.py --gpu 0 --seed 7 --result-dir ./results_gpu0_seed7
```
...
フォルダ名は seed / device により変化します（例: `results_v7_iter1000_batch256_seed1_cuda0`）。  
...
```bash
rsync -avz --progress \
  'wsl-ubuntu:/home/takumi/docker_miniconda/src/PressurePattern/results_v7_iter10000_batch128_seed{1}_*' \
  /Users/takumi0616/Develop/docker_miniconda/src/PressurePattern/result_wsl-ubuntu

rsync -avz --progress \
  'wsl-ubuntu:/home/takumi/docker_miniconda/src/PressurePattern/results_v7_iter10000_batch128_seed1_cuda0' \
  /Users/takumi0616/Develop/docker_miniconda/src/PressurePattern/result_wsl-ubuntu
```
...
```bash
rsync -avz --progress \
  'via-tml2:/home/s233319/docker_miniconda/src/PressurePattern/results_v7_iter1000_batch256_seed{1,2}_*' \
...
```bash
rsync -avz --progress \
  'gpu02:/home/devel/work_takasuka_git/docker_miniconda/src/PressurePattern/results_v7_iter10000_batch128_seed{1}_*' \
  /Users/takumi0616/Develop/docker_miniconda/src/PressurePattern/result_gpu02
```

注意:

- `results_v7_*` のパターンで一括転送できます。個別に指定する場合は実行時に表示された `RESULT_DIR` をそのまま使うと確実です。
...
nohup bash run_seeds.sh --start 1 --end 6 --gpu0 0 --gpu1 1 --channel gpu02 --script main_v7.py > orchestrator.log 2>&1 &

nohup bash run_seeds.sh --start 7 --end 10 --gpu0 0 --gpu1 1 --channel gpu01 --script main_v7.py > orchestrator.log 2>&1 &
```

そのコマンドは正常に動きます。前提として src/PressurePattern に移動してから実行する場合、以下が成立します。

- nohup bash run_seeds.sh --start 21 --end 50 --gpu0 0 --gpu1 1 --channel gpu02 --script main_v7.py > orchestrator.log 2>&1 &
  ...
  - スクリプト内で自身のあるディレクトリに cd するため、ジョブの実行とログの場所は安定（相対パスの main_v7.py も解決可能）

補足:

- 同じ内容をリポジトリルートから実行したい場合は以下でも可:

  - nohup bash src/PressurePattern/run_seeds.sh --start 21 --end 50 --gpu0 0 --gpu1 1 --channel gpu02 --script main_v7.py > src/PressurePattern/orchestrator.log 2>&1 &
...
- 事前条件:
  ...
  - main_v7.py が src/PressurePattern に存在し --gpu/--seed を受け付ける

このコマンドを打った時の中断の仕方は？ (swinunet_env) root@bace913e623d:/app/src/PressurePattern# nohup bash run_seeds.sh --start 21 --end 50 --gpu0 0 --gpu1 1 --channel gpu02 --script main_v7.py > orchestrator.log 2>&1 &
...
- まだ子プロセス（学習ジョブ）が残ってしまった場合は個別停止:
  ...
  - pkill -f 'main_v7.py.\*--gpu 0'
  - pkill -f 'main_v7.py.\*--gpu 1'

強制停止（最後の手段）:

- 正常停止できない場合のみ使用してください

  - pkill -9 -f 'main_v7.py'
...
- 途中まで実行済みなら、次の seed から再開
  ...
  - nohup bash run_seeds.sh --start 35 --end 50 --gpu0 0 --gpu1 1 --channel gpu02 --script main_v7.py > orchestrator.log 2>&1 &

# results_v7 集計スクリプトの使い方（search_results_v7.py）

目的:

- `results_v7` 配下の各 seed 実験ディレクトリにある `evaluation_v7.log` を再帰的に探索し、手法ごとに次を集計します。
...
- `PressurePattern/search_results_v7.py`
...
nohup python search_results_v7.py  --root ./results_v7_iter1000_128 > search_results_v7.log 2>&1 &
  ...
  nohup python search_results_v7.py --nc-report --nc-file prmsl_era5_all_data_seasonal_large.nc --nc-start 1991-01-01 --nc-end 2000-12-31 > search_results_v7.log 2>&1 &
...
- `--root PATH`:
  - 探索対象の `results_v7` ルートを指定（省略時は `search_results_v7.py` と同じディレクトリ直下の `results_v7`）。
