...
try:
    from PressurePattern.minisom_v7 import MiniSom as MultiDistMiniSom  # パッケージ実行（例: python -m PressurePattern.main_v7）
except ModuleNotFoundError:
    try:
        # スクリプト実行（例: /app/src/PressurePattern で python main_v7.py）
        from minisom_v7 import MiniSom as MultiDistMiniSom
    except ModuleNotFoundError:
...
def write_evaluation_summary(learning_root: str, result_root: str, methods: List[Tuple[str, str]]) -> str:
    ...
    処理の流れ:
      ...
      - メソッド名を見出しに付けて連結し、evaluation_v7.logとして保存。
    戻り値:
...
def main():
    """
    概要:
      圧力パターンSOM v7のエントリポイント。データ前処理→各距離手法でのSOM学習→検証→集約レポート作成までを自動実行。
    入力(引数):
      - --seed (int): 乱数シード。
    ...
    処理の流れ:
      ...
      - 複数の距離手法（euclidean/ssim5/s1/…）でSOM学習し、True Medoidや各種図表・CSVを保存。
      - 学習代表（基本ラベル）を用いた検証評価と可視化を実施。
      - 最後に各手法の再現率ブロックを抜粋した集約ログを生成。
    出力:
