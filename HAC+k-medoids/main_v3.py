...
RESULT_DIR = './s1_score_clustering_results' # 結果保存ディレクトリ名を変更
...
def load_and_prepare_data(filepath, start_date, end_date, device):
    ...
    # S1スコア計算のために緯度経度の次元数も返す
    return X_normalized, original_data_numpy, spatial_anomaly_data, lat_coords, lon_coords, d_lat, d_lon, time_stamps, labels

# --- 4. S1スコアおよびクラスタリング関連関数 ---

def calculate_s1_pairwise_batch(x_batch, y_all, d_lat, d_lon, epsilon=EPSILON):
    """
    2つのデータセット間でペアワイズS1スコアを計算する。
    S1スコアは小さいほど類似度が高い。

    Args:
        x_batch (torch.Tensor): 形状 (B, D) のバッチデータ
        y_all (torch.Tensor): 形状 (N, D) の比較対象データ
        d_lat (int): データの緯度次元のサイズ
        d_lon (int): データの経度次元のサイズ
        epsilon (float): ゼロ除算を避けるための微小値

    Returns:
        torch.Tensor: 形状 (B, N) のS1スコアマトリックス
    """
    B, D = x_batch.shape
    N, _ = y_all.shape
    ...
    # 差分マップ D = F - A (この文脈では y_maps - x_maps)
    diff_maps = y_maps - x_maps # ブロードキャストにより (B, N, d_lat, d_lon)

    # x方向とy方向の勾配を有限差分で計算
    # 共通の領域で計算するため、サイズを合わせる
    # grad_x: (B, N, d_lat, d_lon-1) -> (B, N, d_lat-1, d_lon-1)
    # grad_y: (B, N, d_lat-1, d_lon) -> (B, N, d_lat-1, d_lon-1)
...
def find_medoid_torch(cluster_indices, X, batch_size, d_lat, d_lon):
    ...
    # S1スコアは小さいほど良いため、スコアの合計が「最小」になる点をメドイドとする
...
def two_stage_clustering(X, th_merge, all_labels, all_time_stamps, base_labels, batch_size, d_lat, d_lon):
    ...
    while True:
        ...
        # S1スコアは小さいほど類似。対角成分は無限大にしてマージ対象から外す
        s1_matrix.fill_diagonal_(torch.inf)
        # 閾値「より小さい」ペアをマージ候補とする
        merge_candidates = (torch.triu(s1_matrix) < th_merge).nonzero(as_tuple=False)
        
        if merge_candidates.shape[0] == 0:
            logging.info("マージ可能なクラスタペアが見つかりませんでした。クラスタリングを終了します。")
            break

        # S1スコアが「小さい順」にマージするため、昇順でソート
        unique_candidates = [{'pair': tuple(pair.tolist()), 's1': s1_matrix[pair[0], pair[1]].item()} for pair in merge_candidates]
        unique_candidates.sort(key=lambda x: x['s1'], reverse=False)
        ...
        temp_medoids = [find_medoid_torch(c, X, batch_size, d_lat, d_lon) for c in tqdm(clusters, desc="HAC: 一時メドイド計算中", leave=False)]
        ...
        while True:
            ...
            all_s1_to_medoids = torch.zeros((n_samples, len(current_medoids)), device=device)
            ...
            # 各データ点を、S1スコアが「最小」となるメドイドのクラスタに割り当てる
            assignments = torch.argmin(all_s1_to_medoids, dim=1)
            ...
            if any(m is None for m in new_medoids):
                ...
                all_s1_to_medoids_recalc = torch.zeros((n_samples, len(valid_medoid_data)), device=device)
                for i in tqdm(range(0, n_samples, batch_size), desc="k-medoids: 割り当て再計算中", leave=False):
                    end_idx = min(i + batch_size, n_samples)
                    x_batch = X[i:end_idx]
                    s1_scores_batch_recalc = calculate_s1_pairwise_batch(x_batch, valid_medoid_data, d_lat, d_lon)
                    all_s1_to_medoids_recalc[i:end_idx, :] = s1_scores_batch_recalc
                
                assignments = torch.argmin(all_s1_to_medoids_recalc, dim=1)
