# -*- coding: utf-8 -*-
# S1は空間偏差(hPa)を用いる版 + ラベル評価(基本/複合)の両建て + 反復評価の可視化
# main_v4.py
...
# --- ユーティリティ: 複合ラベル抽出 ---
def find_composite_labels(all_labels, base_labels):
    comps = set()
    base_set = set(base_labels)
    for l in all_labels or []:
        if l in base_set:
            continue
        if l is None:
            continue
        ...
        if '+' in s or '-' in s:
            ...
            if len(parts) == 2:
                a = parts[0].strip()
                b = parts[1].strip()
                if a in base_set and b in base_set and a != b:
                    comps.add(f"{a}{connector}{b}")
...
def load_and_prepare_data(filepath, start_date, end_date, device):
    ...
    msl_data = data_period['msl']  # Pa
    n_samples, d_lat, d_lon = msl_data.shape
    
    # hPaに変換
    original_data_numpy = msl_data.values / 100.0  # hPa
    logging.info("各日のデータから空間平均値を減算し、空間偏差データ(hPa)を生成しています...")
    spatial_mean = np.mean(original_data_numpy, axis=(1, 2), keepdims=True)
    spatial_anomaly_data = original_data_numpy - spatial_mean  # hPaの空間偏差

    # S1用テンソル（zスコア正規化はかけない）
    X_s1_hpa = torch.from_numpy(
        spatial_anomaly_data.reshape(n_samples, d_lat * d_lon)
    ).to(device, dtype=torch.float32)

    # 参考: ベクトルのzスコア正規化も別途用意（解析などで使用したい場合に利用可能）
    ...
    X_zscore = torch.from_numpy(normalized_data).to(device, dtype=torch.float32)
    ...
    logging.info(f"データ形状: {n_samples}サンプル, {d_lat}x{d_lon}")
    
    # 返却: S1用, オリジナル(hPa), 空間偏差(hPa), zスコア（参考）
    return X_s1_hpa, original_data_numpy, spatial_anomaly_data, X_zscore, lat_coords, lon_coords, d_lat, d_lon, time_stamps, labels

# --- 4. S1スコア ---
def calculate_s1_pairwise_batch(x_batch, y_all, d_lat, d_lon, epsilon=EPSILON):
    # x_batch, y_all は hPa空間偏差の2Dマップをフラットにしたテンソル
    B, D = x_batch.shape
    N, _ = y_all.shape
...
# --- 5. 分布/評価（ラベル集合を引数に一般化） ---
def analyze_cluster_distribution(clusters, all_labels, all_time_stamps, target_labels, iteration_name):
    ...
    for i, cluster_indices in enumerate(clusters):
        ...
        if all_labels:
            ...
            label_counts = Counter(label for label in cluster_labels if label in target_labels)
            if label_counts:
                logging.info("  - ラベル構成 (評価対象ラベルのみ):")
                ...
                for label, count in sorted_labels:
                    ...
                    logging.info(f"    - {label:<6}: {count:4d} 件 ({percentage:5.1f}%)")
...
def evaluate_clusters_majority_recall(clusters, all_labels, target_labels, iteration_name):
    ...
    confusion_matrix = pd.DataFrame(0, index=target_labels, columns=cluster_names, dtype=int)
    for i, idxs in enumerate(clusters):
        if not idxs:
            continue
        cluster_true_labels = [all_labels[j] for j in idxs if all_labels[j] in target_labels]
        label_counts = Counter(cluster_true_labels)
        for label, count in label_counts.items():
            confusion_matrix.loc[label, cluster_names[i]] = count
    present_labels = [lbl for lbl in target_labels if confusion_matrix.loc[lbl].sum() > 0]
    if len(present_labels) == 0:
        logging.warning("評価対象ラベルに該当するデータがありません。評価をスキップします。")
        return None
    logging.info("【混同行列 (評価対象ラベルのみ)】")
    ...
    for k in range(num_clusters):
        ...
        if col_sum == 0:
            ...
            logging.info(f" - {col:<12}: 代表ラベル=None（評価対象ラベルの出現なし）")
            continue
        ...
        logging.info(f" - {col:<12}: 代表={top_label:<6}  件数={top_count:4d}  シェア={share:5.2f}  | 上位: {top3_str}")
    metrics = {}
    per_label = {}
    logging.info("\n【各ラベルの再現率 (クラスタ多数決ベース)】")
    for lbl in present_labels:
        ...
        logging.info(f" - {lbl:<6}: N={row_sum:4d}, Correct(代表クラスタ群内)={num_correct:4d}, Recall={recall:.4f}, 代表クラスタ={cols_for_lbl if cols_for_lbl else 'なし'}")
    ...
    logging.info(f"Micro Accuracy (多数決, 対象ラベル) = {micro_accuracy:.4f}")
    # ARI/NMI
    ...
    if len(y_true) > 0:
        ...
        logging.info("評価対象サンプルがないため、ARI/NMI は計算しません。")
    logging.info(f"--- {iteration_name} 評価終了 ---\n")
    return metrics

# --- 6. 2段階クラスタリング（MNNマージ + k-medoids）, 履歴を保存 ---
def two_stage_clustering(X, th_merge, batch_size, d_lat, d_lon):
    n_samples = X.shape[0]
    logging.info("2段階クラスタリングを開始します (類似度指標: S1スコア, 入力: hPa空間偏差)...")
    clusters = [[i] for i in range(n_samples)]
    medoids = list(range(n_samples))
    history = []  # 各外側反復の結果を保存（clusters, medoids）
    ...
    return clusters, medoids, history

# --- 7. 可視化 ---
def plot_final_distribution_summary(clusters, all_labels, all_time_stamps, target_labels, save_dir):
    logging.info("最終的な分布の可視化画像を作成中...")
    os.makedirs(save_dir, exist_ok=True)
    num_clusters = len(clusters)
    cluster_names = [f'Cluster {i+1}' for i in range(num_clusters)]
    label_dist_matrix = pd.DataFrame(0, index=cluster_names, columns=target_labels)
    month_dist_matrix = pd.DataFrame(0, index=cluster_names, columns=range(1, 13))
    for i, cluster_indices in enumerate(clusters):
        cluster_name = cluster_names[i]
        if all_labels:
            cluster_true_labels = [all_labels[j] for j in cluster_indices if all_labels[j] in target_labels]
            label_counts = Counter(cluster_true_labels)
            for label, count in label_counts.items():
    ...
    fig, axes = plt.subplots(2, 1, figsize=(max(12, len(target_labels)*0.5), 14))
    fig.suptitle('Final Cluster Distribution Summary', fontsize=20)
    sns.heatmap(label_dist_matrix, ax=axes[0], annot=False, cmap='viridis', linewidths=.5)
...
def save_daily_maps_for_clusters(clusters, spatial_anomaly_data, lat, lon, ts, labels, target_labels, save_dir):
    ...
    for i, cluster_indices in enumerate(clusters):
        if labels:
            label_counts = Counter(l for l in [labels[j] for j in cluster_indices] if l in target_labels)
            dominant_labels[i] = label_counts.most_common(1)[0][0] if label_counts else "Unknown"
        else:
            dominant_labels[i] = ""
...
def plot_final_clusters(medoids, clusters, spatial_anomaly_data, lat_coords, lon_coords, time_stamps, all_labels, target_labels, save_dir, th_merge_value):
    ...
    for i in range(num_clusters):
        ...
        dominant_label_str = ""
        if all_labels and target_labels:
            label_counts = Counter(label for label in [all_labels[j] for j in cluster_indices] if label in target_labels)
            if label_counts:
    ...
    for i in range(num_clusters, len(axes)):
        axes[i].axis("off")
    fig.suptitle(f'Final Synoptic Patterns (Medoids of Spatial Anomaly) - TH_merge={th_merge_value}', fontsize=16)
    fig.tight_layout(rect=[0, 0, 0.9, 0.95])
    cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])
    fig.colorbar(cont, cax=cbar_ax, label="Sea Level Pressure Anomaly (hPa)")
    save_path = os.path.join(save_dir, f'final_clusters_anomaly_th{th_merge_value}.png')
    plt.savefig(save_path)
    plt.close()
    logging.info(f"最終クラスタの画像（空間偏差図）を保存: {save_path}")

# --- 8. 反復ごとの評価プロット ---
def plot_iteration_metrics(history, all_labels, target_labels, out_dir, title_suffix=""):
    """
    history: [{'iteration':k, 'clusters':..., 'medoids':...}, ...]
    各iterationのクラスタ構成で評価を行い、MacroRecall/MicroAcc/ARI/NMIの推移を可視化
    """
    os.makedirs(out_dir, exist_ok=True)
    iters = []
    macro_list, micro_list, ari_list, nmi_list = [], [], [], []
    for h in history:
        it = h['iteration']
        clusters_it = h['clusters']
        metrics = evaluate_clusters_majority_recall(clusters_it, all_labels, target_labels, f"Iteration {it} 評価")
        iters.append(it)
        if metrics is None:
            macro_list.append(np.nan)
            micro_list.append(np.nan)
            ari_list.append(np.nan)
            nmi_list.append(np.nan)
        else:
            macro_list.append(metrics.get('MacroRecall_majority', np.nan))
            micro_list.append(metrics.get('MicroAccuracy_majority', np.nan))
            ari_list.append(metrics.get('ARI_majority', np.nan))
            nmi_list.append(metrics.get('NMI_majority', np.nan))
    # 可視化
    plt.figure(figsize=(10, 8))
    plt.subplot(2, 2, 1)
    plt.plot(iters, macro_list, marker='o')
    plt.title('Macro Recall (majority)')
    plt.xlabel('Iteration'); plt.ylabel('Score'); plt.grid(True)
    plt.subplot(2, 2, 2)
    plt.plot(iters, micro_list, marker='o')
    plt.title('Micro Accuracy (majority)')
    plt.xlabel('Iteration'); plt.ylabel('Score'); plt.grid(True)
    plt.subplot(2, 2, 3)
    plt.plot(iters, ari_list, marker='o')
    plt.title('ARI (majority)')
    plt.xlabel('Iteration'); plt.ylabel('Score'); plt.grid(True)
    plt.subplot(2, 2, 4)
    plt.plot(iters, nmi_list, marker='o')
    plt.title('NMI (majority)')
    plt.xlabel('Iteration'); plt.ylabel('Score'); plt.grid(True)
    plt.suptitle(f'Iteration vs Metrics {title_suffix}', fontsize=14)
    plt.tight_layout(rect=[0,0,1,0.96])
    save_path = os.path.join(out_dir, 'iteration_vs_metrics.png')
    plt.savefig(save_path)
    plt.close()
    logging.info(f"反復-評価スコア関係の図を保存: {save_path}")

# --- 9. ラン一式（指定ラベル集合での出力） ---
def run_outputs_for_labelset(labelset_name, target_labels, clusters, medoids, X_anomaly, lat, lon, ts, labels, base_out_dir, th_merge_value):
    """
    labelset_name: 'only_base_label' or 'multi_label'
    target_labels: 評価に使用するラベル一覧
    """
    out_dir = os.path.join(base_out_dir, labelset_name)
    os.makedirs(out_dir, exist_ok=True)

    # 分布可視化
    if labels and ts is not None:
        plot_final_distribution_summary(clusters, labels, ts, target_labels, out_dir)
    # 日次図
    if X_anomaly is not None:
        save_daily_maps_for_clusters(clusters, X_anomaly, lat, lon, ts, labels, target_labels, out_dir)
    # メドイド図
    plot_final_clusters(medoids, clusters, X_anomaly, lat, lon, ts, labels, target_labels, out_dir, th_merge_value)

# --- 10. メイン ---
if __name__ == '__main__':
    logging.info("--- 2段階クラスタリングプログラム開始 (S1はhPa空間偏差を使用) ---")
    X_s1, X_original, X_anomaly, X_zscore, lat, lon, d_lat, d_lon, ts, labels = load_and_prepare_data(
        DATA_FILE, START_DATE, END_DATE, device
    )

    # S1スコアの分布（hPa空間偏差ベース）
    logging.info("S1スコアの分布を全データで調査します（hPa空間偏差ベース）...")
    X_sample = X_s1
    ...
    # クラスタリング本体（X_s1: hPa空間偏差）
    start_time = time.time()
    final_clusters, final_medoids, history = two_stage_clustering(
        X_s1, TH_MERGE, CALCULATION_BATCH_SIZE, d_lat, d_lon
    )
    ...
    # 評価対象ラベル集合（基本のみ / 基本＋複合）
    composite_labels = find_composite_labels(labels, BASE_LABELS) if labels else []
    logging.info(f"検出された複合ラベル数: {len(composite_labels)} -> {composite_labels}")
    only_base_labels = BASE_LABELS[:]  # コピー
    multi_labels = BASE_LABELS[:] + composite_labels

    # 解析・評価（ログ出力）
    analyze_cluster_distribution(final_clusters, labels, ts, only_base_labels, "最終結果（基本ラベルのみ）")
    if labels:
        _ = evaluate_clusters_majority_recall(final_clusters, labels, only_base_labels, "最終結果（基本ラベルのみ）")
    analyze_cluster_distribution(final_clusters, labels, ts, multi_labels, "最終結果（基本＋複合ラベル）")
    if labels and len(multi_labels) > 0:
        _ = evaluate_clusters_majority_recall(final_clusters, labels, multi_labels, "最終結果（基本＋複合ラベル）")

    # 保存（結果pt）
    results = {'medoid_indices': final_medoids, 'clusters': final_clusters, 'history': history}
    torch.save(results, os.path.join(RESULT_DIR, f'clustering_result_th{TH_MERGE}.pt'))
    logging.info("クラスタリング結果を保存しました（履歴付き）。")

    # 2種類の出力（分布/日次図/メドイド図）
    run_outputs_for_labelset('only_base_label', only_base_labels, final_clusters, final_medoids,
                             X_anomaly, lat, lon, ts, labels, RESULT_DIR, TH_MERGE)
    run_outputs_for_labelset('multi_label', multi_labels, final_clusters, final_medoids,
                             X_anomaly, lat, lon, ts, labels, RESULT_DIR, TH_MERGE)

    # 反復数と評価スコアの関係（2種類）
    if labels:
        plot_iteration_metrics(history, labels, only_base_labels,
                               os.path.join(RESULT_DIR, 'only_base_label'), title_suffix='(Only Base Labels)')
        plot_iteration_metrics(history, labels, multi_labels,
                               os.path.join(RESULT_DIR, 'multi_label'), title_suffix='(Base + Composite Labels)')
