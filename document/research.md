...
### 4) Jiang, Dirks & Luo (2014, Weather and Climate 33:52–75)「SOM によるニュージーランドの総観型分類と気象・大気質データの可視化」
...
- 本研究への示唆
  - 気圧配置の分類において、従来のユークリッド距離よりも S1 スコアと SSIM が優れているという強力な根拠を提供しています。これに基づき、本研究ではこの 2 つの指標を比較対象として導入します。
...
- 本研究への示唆
  - 既存の SOM とは異なるアプローチで、より物理的に意味のある分類を目指す手法です。本研究ではこの先進的な枠組みを参考にし、類似度指標を S1 スコアに変更して実験条件 ④ として導入します。
...
- 目的 5（解釈性と展開可能性の提示）
  - 各方式で得られた代表パターン（とくに medoid）の地上天気との対応や季節頻度・遷移の可視化を提示し、SOM（Jiang et al., 2014）で実証された「関係の地図化」の強みを、構造敏感な類似度と二段階法の導入でどこまで拡張できるかを示す。将来的なモデル評価（Jensen–Shannon 距離）への接続可能性も議論する。
...
- Micro Accuracy: 各クラスタの多数派件数の総和 / 基本ラベル総件数。
- ARI（Adjusted Rand Index）/ NMI（Normalized Mutual Information）: 基本ラベルを持ち、かつクラスタ代表が None でないサンプルのみで算出。
...
- 入出力: すべて SLP 偏差[hPa]。可視化範囲は日本周辺（120–150E, 20–50N）で統一。

### 7. 想定される成果と新規性

- 同一データ・同一評価系で、EUC/SSIM/S1 という類似度の違いが SOM/二段階クラスタリングに与える影響を定量比較できる。S1（二段階）が構造保持・安定性・再現率で優位になるかを検証する。
- 15 基本ラベル＋複合ラベルという現実的なラベル体系に対して、Macro Recall（基本・複合）を用いた公平な比較を提示する。
- メドイドに基づく可視化により、各クラスの“代表的な天気図”を直接提示でき、解釈性（たとえば冬型・梅雨前線・台風経路の配置）を高める。
- 本研究は、筆者の先行研究（SOM ＋ 1km 天気）で示した「クラスタ ⇔ 地上天気」の対応付けを、類似度・クラスタリング手続きの側面から改善し、将来的に地上影響の説明力を高める基盤となる。
...
今後の展望としては、手法 ④ の長所を活かしつつ分類精度を向上させるため、季節ごとにデータを分割してクラスタリングを行うことや、クラスターのマージ閾値の最適化、少数派ラベルに重み付けを行う評価手法の導入などが考えられる。
