...
output_dir_base = "./result_test_acs_circular_grid"  # "random" から "grid" に変更
output_dir = f"{output_dir_base}_{timestamp}"
os.makedirs(output_dir, exist_ok=True)
log_file_path = os.path.join(output_dir, f"grid_search_log_circular_{timestamp}.txt") # "random" から "grid" に変更
...
print(f"ACSモデル (acs.py - 動的クラスタリング - 円形活性化) のIrisデータセットに対するグリッドサーチと評価を開始します。") # "ランダムサーチ" から "グリッドサーチ" に変更
...
# 2. グリッドサーチのためのパラメータグリッドを設定 (円形活性化用)
# 注意: グリッドの点の数はパラメータ候補値の数の積になります。計算時間に注意して調整してください。
# 以下は例です。実際の探索ではより多くの値を試すか、範囲を絞る必要があります。
param_grid = {
...
fixed_params = {
    ...
    'lambda_min_val': 1e-7,
    'bounds_W': (0, 1),
    'random_state': None # 各試行でACSモデルのrandom_stateは別途設定
}
num_epochs = 300 # グリッドサーチ時のエポック数 (調整可能)
...
for trial_count, param_values_tuple in enumerate(all_param_combinations, 1):
    ...
    current_random_state_for_acs = random.randint(0, 1000000) # ACSモデルのシード
    ...
    try:
        ...
        is_better = False
        if best_params_combination is None: # 最初の有効な結果
            is_better = True
...
try:
    ...
    df_path = os.path.join(output_dir, f"grid_search_all_results_circular_{timestamp}.csv") # "random" から "grid" に変更
...
print("\n--- グリッドサーチ完了 ---") # "ランダムサーチ" から "グリッドサーチ" に変更

goal_achieved = False # ループ後に判定
if best_params_combination is not None:
    if best_final_n_clusters == target_n_clusters and best_accuracy_mapped >= target_accuracy:
        ...
        goal_achieved = True
    elif best_accuracy_mapped > -1.0: # 何らかの有効な最良結果がある場合
    ...
    if best_model_params_for_refit is None: # この後の処理に進めないため
        sys.exit()
...
if best_params_combination is not None and best_model_params_for_refit is not None:
    ...
    for epoch in range(final_epochs_refit):
        ...
        if (epoch + 1) % (final_epochs_refit // 10 if final_epochs_refit >=10 else 1) == 0 or epoch == final_epochs_refit -1 : # ログ出力を少し間引く
    ...
    metrics_plot_path = os.path.join(output_dir, f"metrics_vs_epoch_best_dynamic_circular_{timestamp}.png")
    ...
    cm_path = os.path.join(output_dir, f"confusion_matrix_best_dynamic_circular_{timestamp}.png")
    ...
    if final_cluster_centers_pca_visual.shape[0] > 0:
        for acs_center_idx in range(final_cluster_centers_pca_visual.shape[0]):
            ...
            original_acs_label = -1 # マッピング前のACSクラスタラベルを探す
            for k, v in pred_to_true_map.items(): # pred_to_true_map は {acs_label: true_label_idx}
                # final_cluster_centers_pca_visual のインデックスが直接ACSクラスタラベルに対応しているか確認が必要
                # 現状のACS実装では、get_cluster_centers()で得られるWの行インデックスがクラスタラベル
                # よって acs_center_idx がそのまま original_acs_label
            ...
            plt.scatter(final_cluster_centers_pca_visual[acs_center_idx, 0],
                        ...
                        label=f'Center{center_label_suffix}') # ラベルは多すぎると凡例が読みにくくなるので注意
    ...
    by_label = dict(zip(labels, handles)) # 重複ラベルを削除して凡例表示
    plt.legend(by_label.values(), by_label.keys(), title="予測クラス / 中心", fontsize=8, loc="best")
    plt.xticks(fontsize=10)
    plt.yticks(fontsize=10)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    pca_plot_path = os.path.join(output_dir, f"pca_clustering_best_dynamic_circular_{timestamp}.png")
    plt.savefig(pca_plot_path, dpi=300, bbox_inches='tight')
    plt.close(fig_pca_clusters)
    ...
    if best_model_instance.M > 0 and best_model_instance.fitted_:
        ...
        ax_energy.set_title(f'エネルギー関数 E の等高線図 (PCA空間 - 動的ACS - 円形)\n最終Acc: {final_accuracy_refit:.3f}, Cls: {best_model_instance.M}', fontsize=14)
        ax_energy.set_xlabel('PCA特徴量1 (可視化用)', fontsize=12)
        ax_energy.set_ylabel('PCA特徴量2 (可視化用)', fontsize=12)
        handles_e, labels_e = ax_energy.get_legend_handles_labels() # 凡例重複回避
        by_label_e = dict(zip(labels_e, handles_e))
        ax_energy.legend(by_label_e.values(), by_label_e.keys(), fontsize=10, loc='upper right')
        ax_energy.grid(True, linestyle=':', alpha=0.5)
        ax_energy.set_xlim(x_min, x_max)
        ax_energy.set_ylim(y_min, y_max)
        energy_contour_path = os.path.join(output_dir, f"energy_contour_plot_circular_{timestamp}.png") # "circular"追加
    ...
    for k, v_param in best_model_params_for_refit.items():
        if isinstance(v_param, float):
            params_str_list.append(f"'{k}': {v_param:.7g}") # 指数表記も許容しつつ有効桁数を意識
        elif isinstance(v_param, (int, str, bool)) or v_param is None:
            params_str_list.append(f"'{k}': {v_param}")
        elif isinstance(v_param, tuple): # bounds_Wなど
             params_str_list.append(f"'{k}': {v_param}")
        else: # np.ndarrayなど、特殊な表示が必要な場合
             params_str_list.append(f"'{k}': type({type(v_param).__name__})")
...
"""
    ...
    if best_model_instance.M > 0 and n_final_valid_preds > 0:
        ...
        for i in range(best_model_instance.M): # ACSクラスタのインデックスは0からM-1
            mapped_true_class_name = "N/A (Unmapped or Empty)"
            if i in pred_to_true_map: # pred_to_true_map のキーはACSクラスタラベル
        ...
        unassigned_count = counts.get(-1, 0) # 未分類ラベルが-1の場合
    ...
    param_log_path = os.path.join(output_dir, f"best_model_params_dynamic_circular_{timestamp}.txt")
    with open(param_log_path, "w", encoding="utf-8") as f:
        f.write(f"実行開始時刻: {timestamp}\n")
        if goal_achieved:
            f.write("目標達成しました (最良の結果が目標値を満たしています)。\n")
        else:
            f.write("目標未達でした (最良の結果が目標値を満たしていません)。\n")
        ...
        f.write(best_model_summary) # こちらはACSに渡された全パラメータ
