...
# cartopy は clustering+som 側で必須、3type_som 側では省略可のため両対応
try:
...
# ============== clustering+som 側ユーティリティ群のインポート ==============
...
# ============== 3type_som 側のMiniSom（PyTorch実装） ==============
# 注意: s1_minisom の MiniSom と名前衝突を避けるため別名にしています
from minisom import MiniSom as MiniSom3

# ============== ユーザ調整パラメータ（clustering+som 部分） ==============
SEED = 42                 # 乱数シード（再現性）
...
# SOM学習のイテレーション回数（メドイドSOM）
SOM_MAX_ITERS_CAP = 300

# ============== 固定：実験条件・パス等（clustering+som 部分） ==============
...
def setup_logging():
    ...
    log_path = os.path.join(RESULT_DIR, 'run_v1.log')  # パスは既存のものを維持
...
# データ読み込み ＆ 前処理（共通化）
#   - Pa -> hPa
#   - 各サンプルから空間平均を差し引いた偏差[hPa]（anomaly）を作成
#   - タイムコーディネートは 'valid_time' or 'time'
#   - NaNを含むサンプルは全て除外（タイム・ラベルも同様に絞る）
# =====================================================
def load_and_prepare_all(filepath: str):
    """
    ファイル全体を読み込み、以下を返す:
      - data_flat: (N, D) 空間平均を引いた偏差[hPa]
      - msl_hpa_3d: (N, H, W) msl[hPa]
      - anomaly_hpa_3d: (N, H, W) 偏差[hPa]
      - field_shape: (H, W)
      - times: 配列（np.ndarray）
      - lat, lon: 1D座標
    ...
    # hPaに変換し、各サンプルから空間平均を引く（偏差）
    data_hpa = (arr2 / 100.0).astype(np.float32)
    mean_per_sample = np.nanmean(data_hpa, axis=1, keepdims=True)
    data_flat = data_hpa - mean_per_sample           # (N, D) 偏差[hPa]
    msl_hpa_3d = data_hpa.reshape(len(times), nlat, nlon)
    anomaly_hpa_3d = data_flat.reshape(len(times), nlat, nlon)

    field_shape = (nlat, nlon)
    logging.info(f"Loaded: N={data_flat.shape[0]}, field_shape={field_shape}, input_len={data_flat.shape[1]}")
    logging.info('Internal representation: SLP anomaly [hPa], spatial-mean removed per sample')

    return data_flat, msl_hpa_3d, anomaly_hpa_3d, field_shape, times, lat, lon, labels


def subset_by_dates(data_flat: np.ndarray,
                    msl_hpa_3d: np.ndarray,
                    anomaly_hpa_3d: np.ndarray,
                    times: np.ndarray,
                    labels: Optional[List[str]],
                    start_date: Optional[str],
                    end_date: Optional[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Optional[List[str]]]:
    """
    start_date / end_date を用いてデータを抽出して返す。
    None の場合は全期間を返す。
    """
    if (start_date is None) and (end_date is None):
        return data_flat, msl_hpa_3d, anomaly_hpa_3d, times, labels

    tpd = pd.to_datetime(times)
    mask = np.ones(len(times), dtype=bool)
    if start_date is not None:
        mask &= (tpd >= pd.to_datetime(start_date))
    if end_date is not None:
        mask &= (tpd <= pd.to_datetime(end_date))

    data_sub = data_flat[mask]
    msl_sub = msl_hpa_3d[mask]
    anom_sub = anomaly_hpa_3d[mask]
    times_sub = times[mask]
    labels_sub = [labels[i] for i, ok in enumerate(mask) if ok] if labels is not None else None
    return data_sub, msl_sub, anom_sub, times_sub, labels_sub


# =====================================================
# 3type_som 側の補助関数（オリジナルをそのまま）
# =====================================================
...
# =====================================================
# 3type_som: 学習・評価パイプライン（全期間・1方式分）
# =====================================================
...
# =====================================================
# clustering+som 側の補助：クラスタ多数決ラベル（メドイドSOM注釈用）
# =====================================================
...
def main():
    ...
    # -------- 共通データ読み込み（hPa、空間平均差し引き済み）--------
    data_flat_all, msl_hpa_all, anom_hpa_all, field_shape, times_all, lat, lon, labels_all = load_and_prepare_all(DATA_FILE)
    d_lat, d_lon = field_shape

    # =====================================================
    # 1) 3種類の batchSOM（全期間 or 指定期間）
    # =====================================================
    data_3, msl_3, anom_3, times_3, labels_3 = subset_by_dates(
        data_flat_all, msl_hpa_all, anom_hpa_all, times_all, labels_all, TIME_START, TIME_END
    )
    print(f'[3type SOM] Using N={data_3.shape[0]} samples for methods, field_shape={field_shape}')
    if len(times_3) > 0:
        tmin = pd.to_datetime(times_3.min()).strftime('%Y-%m-%d')
        tmax = pd.to_datetime(times_3.max()).strftime('%Y-%m-%d')
        print(f'[3type SOM] Period: {tmin} to {tmax}')
    ...
    for mname, adist in methods:
        ...
        run_one_method(
            ...
            data_all=data_3, labels_all=labels_3, times_all=times_3,
            field_shape=field_shape, lat=lat, lon=lon, out_dir=out_dir
        )

    # =====================================================
    # 2) 二段階クラスタリング（1991-2000 のみ）
    # =====================================================
    # サブセット（clustering+som の期間）
    data_c, msl_c, anom_c, times_c, labels_c = subset_by_dates(
        data_flat_all, msl_hpa_all, anom_hpa_all, times_all, labels_all, START_DATE, END_DATE
    )

    # torchテンソル（S1クラスタリング用）
    X_for_s1 = torch.from_numpy(data_c.astype(np.float32)).to(device, dtype=torch.float32)
    X_anomaly_hpa = anom_c.astype(np.float32)  # (N,H,W)
    X_original_hpa = msl_c.astype(np.float32)  # (N,H,W)

    logging.info(f"期間: {START_DATE} 〜 {END_DATE}, サンプル数={X_for_s1.shape[0]}, 解像度={d_lat}x{d_lon}")
    ...
    # 反復毎の評価履歴（MicroAccuracy, ARI, NMI は削除）
    ...
    clusters, medoids = two_stage_clustering(
        X_for_s1, TH_MERGE, labels_c, times_c, BASE_LABELS,
    ...
    # 最終状態の分析・評価
    analyze_cluster_distribution(clusters, labels_c, times_c, BASE_LABELS, title="最終クラスタ分布分析（基本ラベル）")
    _ = evaluate_clusters_only_base(clusters, labels_c, BASE_LABELS, title="最終評価（基本ラベル）", medoids=medoids)
    ...
    plot_final_distribution_summary(clusters, labels_c, times_c, BASE_LABELS, dist_img)
    ...
    plot_final_clusters_medoids(medoids, clusters, X_anomaly_hpa, lat, lon, times_c, labels_c, BASE_LABELS, final_clusters_img)
    logging.info(f"メドイド図保存: {final_clusters_img}")

    # 日次マップ出力
    daily_dir = os.path.join(ONLY_BASE_DIR, 'daily_anomaly_maps_only_base')
    save_daily_maps_per_cluster(clusters, X_anomaly_hpa, lat, lon, times_c, labels_c, BASE_LABELS, daily_dir, per_cluster_limit=DAILY_MAPS_PER_CLUSTER_LIMIT)
    logging.info(f"日次マップ保存先: {daily_dir}")

    # クラスタ情報をCSVで保存
    info_df = summarize_cluster_info(clusters, medoids, labels_c, BASE_LABELS, times_c)
    ...
    rep_map = compute_cluster_majorities(clusters, labels_c, BASE_LABELS)  # {cluster_idx: rep or None}
    ...
    for k, (x, y) in enumerate(bmu_xy):
        ...
        date_str = pd.to_datetime(str(times_c[mi])).strftime('%Y-%m-%d')
        raw_lbl = labels_c[mi] if labels_c else None
    ...
    # SOMコードブック（各ノードのパターン）地理図でタイル表示（割当なしノードは非表示）
