...
def set_reproducibility(seed: int = 42):
    ...
    if torch.cuda.is_available():
        ...
        # Optional speed-up: enable TF32 via env ENABLE_TF32=1
        if os.getenv('ENABLE_TF32', '0') == '1':
...
def setup_logging_v8():
    ...
    logging.info("ログ初期化完了（run_v8.log）。")
...
def run_one_method_learning(method_name, activation_distance, data_all, labels_all, times_all,
    ...
    """
    学習（learning）：method_name: 'euclidean' | 'ssim5' | 's1' | 'kappa' | 's1k' | 'emd'
    ...
    try:
        ...
        setattr(som, 'persist_cache_dir', cache_dir)
    except Exception:
        pass
